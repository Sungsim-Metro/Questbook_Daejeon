        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                if path_parts == ["api", "me", "nickname"]:
                    ...
                    return
                if path_parts == ["api", "me", "ggumdori"]:
                    # 변수 의미: 토큰에서 검증한 사용자 ID다.
                    user_id = self._required_user_id()
                    # 변수 의미: 대표 꿈돌이 식별자를 담은 요청 본문이다.
                    payload = self._read_json_body()
                    self._send_json(HTTPStatus.OK, state.service.select_ggumdori(user_id, payload))
                    return
