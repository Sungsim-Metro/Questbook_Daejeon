...
- 한국 날짜 기준 1일 1회 전체 관광지 수집 및 공용 퀘스트 사전 생성·변경분 갱신
- 정상 API 관광지와 저장 퀘스트의 `contentId` 연결 및 퀘스트 없는 관광지 제외
- 반복된 정상 전체 조회의 누락 표시·삭제 유예와 수락·완료 사용자 사본 보존
- `UserQuestInstance` 사용자별 퀘스트 상태
- GPS 반경 기반 퀘스트 완료 처리
- XP, 레벨, 뱃지, 수첩 기록 갱신 트랜잭션
- 꿈돌이 도감 해금 상태 조회
- NCP Object Storage presigned 사진 증빙 업로드·다운로드 URL 발급 경계
- 사용자 직접 관심 카테고리 복수 선택·저장과 신규 사용자 미설정 안내
- 주요 지점·주소·지도·좌표로 추천 기준점을 정하는 계획 모드
- 현재 GPS와 계획 지점의 분리 및 현장 GPS 완료 인증 유지
- 대전 지역 관광지 조회와 관심사 우선 추천, 관광지 주변 퀘스트 연결

### 계획 모드 사용

좌측 상단 메뉴에서 프로필을 확인하고 선호 카테고리를 저장하거나 모드를 변경한다. 기본 탐험 모드는 기존 홈과 현재 위치 추천 흐름을 유지한다. 계획 모드는 별도 화면의 위치로 찾기와 선호 카테고리로 찾기로 나뉜다. GPS 권한 없이 계획 지점을 설정하고 퀘스트·지도에서 같은 기준의 결과를 확인할 수 있다. 대전 관광지 추천은 위치 입력 없이 사용하며, 각 카드에서 같은 카테고리의 주변 퀘스트로 이동한다. 메뉴에서 탐험 모드로 복귀하면 기존 탐험 위치와 필터를 다시 사용한다. 주소 검색에는 기존 NAVER REST 설정이 필요하고 주요 지점·좌표 입력은 지도 설정 없이도 사용할 수 있다.

관심사와 공용 퀘스트, 관광지 최소 참조·누락 관측 상태는 PostgreSQL에 저장한다. 관광지 원본 전체와 전역 위치 검색용 좌표 DB는 저장하지 않으며, 수락한 사용자의 관광지 좌표는 개인 퀘스트 사본에 보존한다. 대전 지역 화면 조회는 최대 5페이지와 총 대기 시간 제한을 적용하며 부분 조회·예시 데이터·정상 빈 결과를 구분한다. 신규 사용자는 관심사 빈 배열로 시작하고 기존 계정의 저장 값은 보존한다. 완료 인증에는 계획 지점 대신 현장 실측 GPS를 사용한다.

2026-09-08 카탈로그 변경: 정상 추천에서 공용 퀘스트를 즉시 생성하지 않고 일일 작업이 사전 생성한 정의를 조회한다. `catalog-sync`는 전체 페이지 완결성을 검증하며 실패·부분 조회는 누락 및 삭제에 반영하지 않는다. 기본 삭제 유예는 7일이며 서로 다른 정상 관측일 8회 이상 누락을 요구한다. 공용 수정·삭제와 무관하게 수락·완료 내용 및 수첩을 유지한다. 사용자 요청에 따라 API 실패의 예시 폴백과 기존 연결 정책은 변경하지 않았다. 운영 절차는 `quest-catalog-operations.md`를 참고한다.

2026-09-08 카탈로그 검증: Python API·저장소·HTTP 스모크 221건과 하위 사례 77건, JavaScript 행동 테스트 26건 통과. 실제 PostgreSQL의 동시 완료·수락/추천 경합, 최초 수집부터 누락된 기존 정의의 편입·삭제, 명시적 마이그레이션 반복 적용, 공용 변경·삭제 이후 사용자 기록과 보상 보존을 검증했다. Compose 설정과 CLI 도움말도 통과했다. 외부 관광 API는 통제된 응답으로 검증했으며 서비스 재시작과 운영 DB 반영은 수행하지 않았다.

2026-09-06 검증 결과: Python API·저장소·HTTP 스모크 123건과 하위 사례 77건, JavaScript 행동 테스트 14건 통과. 360px 브라우저에서 관심사 저장·새로고침 유지, 계획 지점 변경, 빈 결과 지도, 관광지에서 주변 퀘스트로 이동을 확인했다. 실제 TourAPI/NAVER 자격증명을 사용하는 외부 연동은 이번 검증에 포함하지 않았으며, 원천 응답을 통제한 테스트와 지도 미설정 대체 흐름을 검증했다.

## 2. 구현된 화면

2026-09-06 메뉴 UI 개편 검증: JavaScript 행동 테스트 26건 통과. 360px 화면에서 메뉴·프로필·선호도 저장, Escape·배경 클릭 닫기와 포커스 복귀, 계획 위치 변경, 선호 관광지의 같은 카테고리 퀘스트 연결, 탐험 모드 복귀를 확인했다. 탐험 모드에서는 대전 전체 관광지를 조회하지 않는다.
...
baseline 분리 구현은 현재 PostgreSQL에 사용자, 선호도, 레벨, 뱃지, 공용 퀘스트, 관광지 최소 참조·일일 관측, 사용자별 퀘스트 사본, 완료 기록, 수첩 기록, 꿈돌이 해금 상태를 저장한다. 실시간 TourAPI 장소 후보는 앱 서버의 Redis 캐시에 유저 단위 30분 TTL로 보관하고 원본 전체를 영구 저장하지 않는다. 사진 증빙 원본은 NCP Object Storage 비공개 버킷을 전제로 presigned URL 발급 경계만 준비되어 있으며, DB에는 `photo_ref` 객체 키만 저장한다.
