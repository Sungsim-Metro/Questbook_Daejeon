...
baseline 사용자 홈에서 관심 카테고리를 저장한 뒤 계획 모드를 선택한다. GPS 권한 없이 계획 지점을 설정하고 퀘스트·지도에서 같은 기준의 결과를 확인할 수 있다. 대전 관광지 추천은 위치 입력 없이 사용하며, 각 카드에서 주변 퀘스트로 이동한다. 주소 검색에는 기존 NAVER REST 설정이 필요하고 주요 지점·좌표 입력은 지도 설정 없이도 사용할 수 있다.

관심사는 PostgreSQL에 저장되지만 관광지 원본 및 좌표는 영구 저장하지 않는다. 대전 지역 조회는 최대 5페이지와 총 대기 시간 제한을 적용하며 부분 조회·예시 데이터·정상 빈 결과를 구분한다. 신규 사용자는 관심사 빈 배열로 시작하고 기존 계정의 저장 값은 보존한다. 완료 인증에는 계획 지점 대신 현장 실측 GPS를 사용한다.
