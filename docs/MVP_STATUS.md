...
baseline 구현은 사용자 PWA, 웹 게이트웨이, Python 앱 API, SQLite 관계형 저장소, 앱 서버 인메모리 캐시를 포함한다. 한국관광공사 TourAPI 키가 없으면 대전 fallback 장소 후보로 흐름을 검증하고, 키가 있으면 앱 서버가 TourAPI를 호출한다. OpenAPI 원본 응답 전체는 영구 DB에 저장하지 않고, 추천 계산에 필요한 최소 필드만 유저 단위 30분 인메모리 캐시에 둔다.
...
- SQLite 관계형 저장소 baseline
- 유저 단위 30분 TourAPI 임시 캐시
...
- `services/app-api/src/questbook_api/infrastructure/repository.py`: SQLite 스키마, seed, 트랜잭션
- `services/app-api/src/questbook_api/infrastructure/cache.py`: 유저 단위 30분 인메모리 캐시
...
- `scripts/backup_sqlite.py`: SQLite baseline 백업 스크립트
...
baseline 분리 구현은 SQLite에 사용자, 선호도, 레벨, 뱃지, 공용 퀘스트, 사용자별 퀘스트 인스턴스, 완료 기록, 수첩 기록, 꿈돌이 해금 상태를 저장한다. TourAPI 장소 후보는 영구 저장하지 않고 앱 서버 인메모리 캐시에만 보관한다.
...
- SQLite 개발 저장소를 운영 DB 엔진에 맞는 마이그레이션 도구로 이전한다.
