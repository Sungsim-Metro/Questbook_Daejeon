...
로컬 개발 DB에 직접 적용할 때는 compose 스택을 실행한 뒤 다음 명령을 사용한다. 신규 DB는 번호 순서대로 `001_baseline_schema.sql`, `002_adventure_note_entries.sql`, `003_user_preferences.sql`을 적용한다. 추가 마이그레이션은 멱등 변경이므로 최신 `001`을 적용한 신규 DB에도 안전하게 다시 적용할 수 있다.
...
docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' < database/migrations/003_user_preferences.sql
```
...
`003_user_preferences.sql`은 `preferences.categories_set_at`을 추가해 직접 설정한 관심사를 구분한다. 기존 관심사 배열은 변경하지 않는다. 기존 운영 DB는 스키마를 삭제하지 않고 다음 변경만 적용한다.

```bash
docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1' < database/migrations/003_user_preferences.sql
```

앱 API 시작 시에도 내장 스키마가 같은 컬럼을 멱등 추가한다. 현재 루트 Compose의 app/web은 이미지에 소스를 포함하므로 코드 반영에는 `docker compose up -d --build app web`이 필요하다. `uv` 개발 실행은 프로세스를 다시 시작한다.
## main UI 통합의 마이그레이션 범위

현재 서버는 기존 카테고리 XP 보상을 유지한다. 사용자 관심사와 카탈로그 스냅샷은 `003_user_preferences.sql`, `004_quest_catalog.sql` 및 저장소 초기화 코드로 관리한다. 서버 시작 시 초기화 적용 경로와 배포 순서는 [카탈로그 운영 문서](../docs/quest-catalog-operations.md)를 따른다.

main에 있던 퀘스트별 고유 보상 SQL은 `proposals/quest_reward_pairs_v2.sql`로 이동했다. 기존 003 관심사 마이그레이션과 번호가 겹치고 현재 보상 정책과도 다르므로, 일반 마이그레이션 실행 목록에 포함하지 않는다.
