...
- `docs/quest-catalog-operations.md`: 퀘스트 일일 갱신, 누락 관광지 삭제 유예, 사용자 사본 보존과 배포 절차
...
좌측 상단 메뉴의 선호 카테고리에서 자연 관찰, 과학 문화, 원도심 걷기, 지역 상권, 이동형, 야경 기록의 관심 카테고리를 여러 개 선택해 저장할 수 있다. 신규 사용자도 활동 이력 없이 추천을 시작하고, 기존 사용자는 관심사를 언제든 변경할 수 있다.

기본 탐험 모드에서는 기존 홈·지도·퀘스트 흐름과 현재 위치 기준 추천을 유지한다. 메뉴의 모드 변경에서 계획 모드를 선택하면 별도 계획 화면으로 이동한다. 위치로 찾기에서는 주요 지점, 주소 검색, 지도 클릭 또는 위경도 입력으로 정한 위치를 기준으로 퀘스트를 추천한다. 계획 위치는 실제 GPS와 별도로 관리하며, 퀘스트 완료 시에는 현장에서 측정한 GPS로 인증한다.

계획 화면의 선호 카테고리로 찾기는 현재 위치와 상관없이 대전 지역을 조회하고 저장된 선호 카테고리의 관광지를 표시한다. 전체 또는 개별 주제로도 바꿀 수 있다. 각 관광지에서 같은 카테고리의 주변 퀘스트로 이동한다. 관심사가 비어 있으면 일반 목록을 제공한다. TourAPI 키가 없거나 장애가 있으면 예시 장소라는 출처를 표시한다.

메뉴 팝업에서 프로필과 탐험 기록을 확인하고 관심사를 변경할 수 있다. 닫기 버튼, Escape 또는 배경 클릭으로 기존 화면에 복귀한다. 계획 모드에서 하단 계획 탭은 계획 화면으로 돌아오며, 메뉴에서 탐험 모드로 바꾸면 이전 탐험 위치와 필터를 다시 사용한다. 대전 전체 관광지는 계획 화면의 선호 탐색을 열 때만 조회한다.

실시간 관광지 후보는 기존 30분 Redis 캐시를 사용한다. 정상 조회에서는 해당 관광지의 저장된 퀘스트가 있을 때만 표시하며, 관광지 식별자·이름·분류와 사전 생성 퀘스트는 하루 한 번 갱신한다. 수락한 퀘스트와 관광지 최소 정보는 사용자별 사본으로 보존한다. 대전 지역 화면 조회는 최대 5페이지를 수집하고 제한 내 일부만 가져왔을 때 안내하며, 백그라운드 갱신은 별도 전체 조회를 사용한다. 기존 주변 퀘스트 점수에는 거리, 관심사, 뱃지와 완료 이력도 반영한다.
...
전체 스택의 `catalog-sync` 서비스는 한국 날짜 기준 하루 한 번 관광지 전체를 확인하고 퀘스트를 준비한다. 최초 정상 수집 전에는 정상 API 관광지에 대응하는 퀘스트가 없어 목록이 비어 있을 수 있다. 기존 키 미설정·API 오류의 예시 폴백은 유지한다. 단발 실행과 삭제 유예 설정은 [카탈로그 운영 문서](docs/quest-catalog-operations.md)를 따른다.
